		...
		if (args != null && args.length > 0) {
			...
			addBehaviour(new TickerBehaviour(this, 60000) {
				protected void onTick() {
					System.out.println("Trying to buy "+targetProduct);
			...
			case 1:
				...
				if (reply != null) {
					...
					if (reply.getPerformative() == ACLMessage.PROPOSE) {
						...
						String[] res = reply.getContent().split("\\+");
						int priceb = Integer.parseInt(res[0]);
						int msi = Integer.parseInt(res[1]);
						String bank = res[2];
						int price=priceb/msi;
						System.out.println("Cx "+getLocalName()+": Recibi de "+reply.getSender()+" el precio por $"+price);
						if(msi>1)
							System.out.println("Cx "+getLocalName()+": Ademas esta a "+msi+" meses sin intereses con tarjetas "+bank+"|||Pagando asi por mes "+price);
